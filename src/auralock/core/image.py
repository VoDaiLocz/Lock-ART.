...
def quantize_image_tensor(tensor: torch.Tensor) -> torch.Tensor:
    """Round-trip a tensor through 8-bit RGB export to match saved file output."""
    image = tensor_to_image(tensor)
    quantized = image_to_tensor(image)
    if tensor.dim() == 4:
